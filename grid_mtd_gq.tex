...
\end{center}
\caption{Illustration of the bipartite graph $\mathcal{G}_B$ and the initialization result. Upper subfigure: construction of $\mathcal{G}_B$. Lower subfigure: the initialization of SP grid. The dark grid cells which do not have corresponding SPs are assigned dummy nodes. }
\label{fig:bg}
\end{figure}
...
\label{alg:hybridDP}
   ...
   \Output Optimal MTD grid $\hat{\mathcal{L}}$
   \Statex
   ...
   \EndFor
   \DoWhile{$\mathcal{L}^{(t)} \neq \mathcal{L}^{(t-1)}$}
   \State Set optimal MTD grid $\hat{\mathcal{L}}=\mathcal{L}^{(t)}$;
\end{algorithmic}
   \end{small}
\end{algorithm}
...
\begin{tabular}{|c|l|c|c|l|}
        \hline
            ...
            \cline{3-5}
            \multicolumn{2}{|c|}{dissimilarity} & {$\omega_f$} &$[1,50]$& {\specialcell[c]{Weight for appe-\\arance dissimilarity}} \\
            \cline{3-5}
            \multicolumn{2}{|c|}{between SPs} & {$\omega_e$}&$[1,50]$ & {\specialcell[c]{Weight for average \\intensity of edges}} \\
            \cline{1-5}
            \multicolumn{2}{|c|}{Superpixel} & {$\beta$}&$[100,200]$ & {\scriptsize\specialcell[c]{ Weight for distance bet-\\ween regional  centroid \\and grid cell Eq. (\ref{eq:pidev})}} \\
            \cline{3-5}
            \multicolumn{2}{|c|}{gridization} & {$\alpha$}&$[0-1]$ & {\specialcell[c]{Weight for structural \\ discrepancy Eq. (\ref{eq:Gtd})} } \\
            \hline
        \end{tabular}
\end{table}
...
\caption{\small The variations of best parameters of (a) MTD-SLIC and (b) MTD-EGS (all parameters are normalized to $[0, 1]$).}
\end{figure}
